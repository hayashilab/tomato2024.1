...
class DetectionPublisher(Node):
    def __init__(self):
        ...
        self.stem_publisher_ = self.create_publisher(Point, 'stem_centroid', 10)
        ...
        self.model = YOLO("stem_tomato.pt")
    ...
    def timer_callback(self):
        ...
        target_classes = {"0", "1", "2", "3"}  # 0 = green tomato, 1 = orange tomato, 2 = red tomato, 3 = stem
        ...
        if len(results[0].boxes) > 0:
            ...
            conf_threshold = 0.00000007  # Adjust confidence threshold
            ...
            for box, cls, conf in filtered_boxes:
                ...
                if class_name in target_classes and class_name not in detected_classes:
                    ...
                    if class_name == "3":
                        ...
                        self.get_logger().info(f"Centroid of {class_name}: ({cx_stem}, {cy_stem})")
    ...
    def publish_stem_centroid(self, cx, cy):
        point_msg = Point()
        point_msg.x = float(cx)
        point_msg.y = float(cy)
        point_msg.z = 0.0
        self.stem_publisher_.publish(point_msg)
        self.get_logger().info(f"Published stem centroid: cx={cx}, cy={cy}")
